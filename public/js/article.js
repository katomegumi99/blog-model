$(function () {
    ...
    function createArticles(articles) {
        ...
        if (articles) {
            articles.forEach(function (article) {
                ...
                <!-- 评论图标 -->
                <i class="fas fa-comment" onclick="addComment(event,${article.id})"></i>评论${article.comments_count}
                </div>
            </li>`
    ...
    window.loadFavoriteArticles = function () {
        $.post("/api/article/favoriteArticleList", { user_id: localStorage.getItem("user_id") }, function (data) {
            let { articleList } = data;
            $(".center").empty();
            $('.center').append($("<ul class='article-list'></ul>"))
            createArticles(articleList);
